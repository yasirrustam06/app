...
#     #     image,
#     #     width=400,  # Manually adjust the width of the image as per requirement
#     #     caption="MediTi Logo"  # Add a non-empty label for accessibility
#     # # )
...
#     with col1:
#         st.subheader("Upload an Image")
#         uploaded_file = st.file_uploader("&nbsp", type=["jpg", "jpeg", "png"], accept_multiple_files=False, help="",label_visibility="hidden")
#         if uploaded_file is not None:
#             image_path = process_uploaded_image(uploaded_file)
...
#         # Initialize ORB image1.shape == image2.shape:
#         orb = cv2.ORB_create()
...
#         # Draw the matches
#         final_image = cv2.drawMatchesKnn(image1, keypoints1, image2, keypoints2, good_matches, None,
...
#             st.markdown("<div style='text-align: center; background-color: white; padding: 5px; border-radius: 5px'><strong><span style='color: black'>Images are the Match.</span></strong></div>", unsafe_allow_html=True)
#         else:
#             st.markdown(f"<div style='display: flex; justify-content: center;'><div style='text-align: center; background-color: white; padding: 5px 20px; border-radius: 5px; max-width: {200}px'><strong><span style='color: black'>Images don't Match.</span></strong></div></div>", unsafe_allow_html=True)
...
def main():
    ...
    if st.button("Take Snapshot"):
        ...
        if ret:
            ...
            st.error("Failed to capture the image from the camera. Please make sure the camera is accessible.")
